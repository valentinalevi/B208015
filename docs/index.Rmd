...
# Make column headers bold to stand out 
    tab_style(
      style = cell_text(weight = "bold"),
      locations = cells_column_labels(everything())
    ) %>%
# Make Season and Average Sunlight hours colums bold too
    tab_style(
      style = cell_text(weight = "bold"),
      locations = cells_body(columns = c(Season, avg_sunlight_hours))
    ) %>%
# Add row striping to make the table more readable
    opt_row_striping() %>%
# Adjust the font size and align headings
    tab_options(
...
northern_gt_table <- gt_table(
  ...
  "Northern Scotland NHS Healthboards Average Antidepressant Prescription Data per Person in 2023"
)
...
southern_gt_table <- gt_table(
  ...
  "Southern Scotland NHS Healthboards Antidepressant Prescription Data per Person in 2023"
)
...
line_plot <- prescription_rates %>%
  ...
  geom_point(size = 1.5) +
  scale_color_manual(values = bright_colors, name = "Health Board") + # Using the custom colour palette for the healthboards
